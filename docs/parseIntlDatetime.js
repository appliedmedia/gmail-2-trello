 ...
 */
Gmail2Trello.App.prototype.browserLanguage = function () {
 ...
 */
Gmail2Trello.App.prototype.parseInternationalDateTime = function (dateTime_in) {
    if (!dateTime_in) {
        return "";
    }
    ...
    while (
        ...
        const value_k =
            match_k && match_k[1] ? ("   " + match_k[1]).substr(-3) : "";
        if (
            value_k &&
    ...
    $.each(["hour", "minutes", "year", "day"], function (iter, item) {
        ...
        const match_k = dateTime.match(rx_k);
        const value_k =
            match_k && match_k.length > 0 && match_k[1] ? match_k[1] : "";
        const value2_k =
            match_k && match_k.length > 1 && match_k[2] ? match_k[2] : "";
        if (value_k) {
            if (value2_k) {
