...
class PopupView {
  ...
  updateMembers() {
    let self = this;
    let members = this.data.trello.members;
    let $g2t = $('#g2tMembers', this.$popup);
    $g2t.html(''); // Clear out
  ...
  validateData() {
    ...
    const emailId = $('#g2tEmailId', self.$popup).val() || 0;
    const boardId = $('#g2tBoard', self.$popup).val();
    const listId = $('#g2tList', self.$popup).val();
    const cardId = $('#g2tCard', self.$popup).val();
    const cardPos = $('#g2tCard', self.$popup).prop('pos') || 0;
    const cardMembers = $('#g2tCard', self.$popup).prop('members') || '';
    const cardLabels = $('#g2tCard', self.$popup).prop('labels') || '';
    ...
    const popupWidth = self.$popup.width();
