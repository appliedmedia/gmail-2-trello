...
Gmail2Trello.PopupView.prototype.updateBody = function (data = {}) {
    ...
    const markdown_k = data.settings?.markdown ?? $("#chkMarkdown", self.$popup).is(":checked");
    const useBackLink_k = data.settings?.useBackLink ?? $("#chkBackLink", self.$popup).is(":checked");
    const addCC_k = data.settings?.addCC ?? $("#chkCC", self.$popup).is(":checked");
