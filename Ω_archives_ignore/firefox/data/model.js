...
Gmail2Trello.Model.prototype.retrieveSettings = function () {
  ...
  Trello.get('members/me', {}, function (data) {
    ...
    if (data.hasOwnProperty('idBoards') && data.idBoards.length > 0) {
      ...
      Trello.get(
        ...
        function (data) {
          for (var i = 0; i < data.length; i++) {
            if (data[i].idOrganization === null) data[i].idOrganization = -1;
          }
          log(data);
...
Gmail2Trello.Model.prototype.submit = function () {
  ...
  if (data.useBacklink) {
    ...
    var txtDirect = `[${email}](${document.location.href} "Direct link to creator's email, not accessible from anyone else")`;
...
Gmail2Trello.Model.prototype.init = function () {
  ...
  logEnabled = true;
  if (self.settings) {
    if (self.settings.orgId == '-1') self.settings.orgId = 'all';
